    ...
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
      ...
      <div class="p-6">
        ...
        <div v-if="selectedParticipants.length > 0" class="mb-4">
          <div class="text-sm font-medium text-gray-700 mb-2">Selected participants:</div>
          <div class="flex flex-wrap gap-2">
            <div
              v-for="participant in selectedParticipants"
              :key="participant.id"
              class="flex items-center space-x-2 bg-[#caf0f8] text-[#014f86] px-3 py-1 rounded-full text-sm"
            >
        ...
        <!-- Search results -->
        <div v-else-if="searchResults.length > 0" class="space-y-2 max-h-48 overflow-y-auto border border-gray-200 rounded-lg">
          <div
        ...
        <div class="mt-6 p-3 bg-gray-50 rounded-lg">
          <div class="text-sm font-medium text-gray-700 mb-2">Current participants:</div>
      ...
      <div class="p-6 border-t border-gray-200 flex justify-end space-x-3">
        ...
        <button
          ...
          :disabled="selectedParticipants.length === 0 || isAdding"
          class="px-4 py-2 hover:cursor-pointer bg-[#2c7da0] text-white rounded-lg hover:bg-[#2a6f97] disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors flex items-center space-x-2"
        >
          <span v-if="isAdding">
            <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
          </span>
          <span v-else>Add Participants</span>
        </button>
      </div>
    </div>
  </div>
...
const filteredSearchResults = computed(() => {
  return searchResults.value.filter(user => 
    ...
    !props.conversation.participants.some(p => p.id === user.id)
  )
})
...
const addParticipantsToGroup = async () => {
  ...
  try {
    ...
    const participantEmails = selectedParticipants.value.map(p => p.email)
    await emit('add', participantEmails)
  } catch (error) {
    console.error('Add participants error:', error)
    errorMessage.value = 'Failed to add participants. Please try again.'
  } finally {
